<!DOCTYPE html>
<html lang="ja">

<head>
  <?php get_header(); ?>
</head>

<body>
  <div class="superwrapper animsition">
    <div class="svg-animation">
      <svg id="レイヤー_1" data-name="レイヤー 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 380.06 99.83"
        width="380.05999755859375" height="99.83000183105469">
        <path class="cls-1 svg-elem-1"
          d="M146.45,384.68c4.32,1.33,8.57,2.9,12.75,4.65a65.91,65.91,0,0,1,8.17-7.14,44.33,44.33,0,0,1,7.92-4.7L158.75,368l-24,13.83A102.37,102.37,0,0,1,146.45,384.68Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-2"
          d="M136.63,414.67q1.71.75,3.39,1.59c.3-.61.62-1.22.94-1.82a116.54,116.54,0,0,1,11.56-17.54c1.57-2,3.2-3.87,4.91-5.72a129.6,129.6,0,0,0-14.7-5.09,91.73,91.73,0,0,0-11.21-2.42L116,392.62v16.15A74.49,74.49,0,0,1,136.63,414.67Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-3"
          d="M122.23,432a44.66,44.66,0,0,1,10.36,5.56c.27-1.47.59-2.93.95-4.38A77.21,77.21,0,0,1,139,418.39c-2-1-4.06-1.91-6.15-2.74A71,71,0,0,0,116,411.21v19.22A31,31,0,0,1,122.23,432Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-4"
          d="M130.17,438.87a40.15,40.15,0,0,0-9.76-4.93,28.48,28.48,0,0,0-4.39-1.1V442l15.28,8.82a75.2,75.2,0,0,1,.84-10.5C131.49,439.79,130.84,439.32,130.17,438.87Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-5"
          d="M173.07,395.9c2.05,1.08,4.08,2.21,6.1,3.37a66.2,66.2,0,0,1,10.93-10.63c.65-.49,1.32-1,2-1.43l-14.2-8.2a40,40,0,0,0-11,6.58,70.29,70.29,0,0,0-5.3,4.76C165.47,392.06,169.31,393.92,173.07,395.9Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-6"
          d="M159.32,428.62c.46.36.91.73,1.36,1.1.73-1.73,1.51-3.44,2.32-5.13a127.84,127.84,0,0,1,11.62-19.45q1.45-2,3-4-4.41-2.53-9-4.83-4.38-2.2-8.9-4.15a98.28,98.28,0,0,0-7,8.26,114.24,114.24,0,0,0-10.66,16.91A111.56,111.56,0,0,1,159.32,428.62Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-7"
          d="M144.81,455.43A38.22,38.22,0,0,0,134.31,442a70.31,70.31,0,0,0-.62,10.15l12.48,7.21A23.44,23.44,0,0,0,144.81,455.43Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-8"
          d="M155.15,428.43a108,108,0,0,0-14.07-9,76,76,0,0,0-5.91,17.34c-.16.8-.31,1.61-.44,2.41a49,49,0,0,1,6.86,6.72c3.52,4.23,6.57,9.47,7.33,15l5.92,3.42c-.24-1.94-.39-3.9-.44-5.85a61.45,61.45,0,0,1,1.32-14.12,82,82,0,0,1,3.47-10.92c.18-.49.37-1,.55-1.45Q157.49,430.19,155.15,428.43Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-9"
          d="M173.09,444.66a68.79,68.79,0,0,0-7.89-7.94c-1.15-1.05-2.33-2.07-3.52-3.07-.28.74-.54,1.48-.79,2.23-.62,1.86-1.4,3.65-1.95,5.53a51.23,51.23,0,0,0-1.42,6.36,58.27,58.27,0,0,0-.56,13.9c.11,1.41.29,2.82.53,4.22l1.26.74,21-12.12A81.62,81.62,0,0,0,173.09,444.66Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-10"
          d="M194,411.72c-4.62-3.33-9.41-6.45-14.32-9.35-1.71,2.14-3.33,4.35-4.88,6.61-2.15,3.13-4.16,6.37-6,9.68-1.7,3-3.33,6.16-4.7,9.17-.53,1.15-1,2.31-1.51,3.47q3.54,3,6.89,6.13c1.42,1.35,2.89,2.68,4.17,4.16,1,1.13,1.89,2.31,2.77,3.52,1.92,2.63,3.71,5.38,5.37,8.21L201.48,442V417.38Q197.84,414.46,194,411.72Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-1 svg-elem-11"
          d="M198.25,411.84c1.09.82,2.16,1.65,3.23,2.49V392.62l-7.06-4.08a43.45,43.45,0,0,0-5,3.7,70,70,0,0,0-8.17,8.26A185.87,185.87,0,0,1,198.25,411.84Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-12"
          d="M251.48,419.25a13.64,13.64,0,0,1-6,5.1l8.7,17.46H247.4l-8.22-16a12.65,12.65,0,0,1-1.44.06h-6.36l-2,16h-5.82l5.34-43.38h11.52q6.24,0,10,3a10.18,10.18,0,0,1,3.78,8.46q0,.66-.12,2A17.25,17.25,0,0,1,251.48,419.25Zm-13.14,1a10.1,10.1,0,0,0,5.73-1.47,8.62,8.62,0,0,0,3.15-3.57,9.78,9.78,0,0,0,1-4.08,7.33,7.33,0,0,0-1.83-5q-1.83-2.1-5.61-2.1H234l-2,16.2Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-13"
          d="M270.56,417l-3.06,24.84h-5.64L264.92,417Zm-4.17-10.59a3.38,3.38,0,0,1,2.49-1,3.63,3.63,0,0,1,2.25.75,2.45,2.45,0,0,1,1,2.07,3.25,3.25,0,0,1-1,2.43,3.48,3.48,0,0,1-2.52,1,3.75,3.75,0,0,1-2.25-.81,2.51,2.51,0,0,1-.93-2.07A3.23,3.23,0,0,1,266.39,406.38Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-14" d="M303.74,417l-16.38,25.56h-.24L276.38,417H283l5.58,15.72L297.5,417Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-15"
          d="M354.08,419.25a13.64,13.64,0,0,1-6,5.1l8.7,17.46H350l-8.22-16a12.65,12.65,0,0,1-1.44.06H334l-2,16h-5.82l5.34-43.38H343q6.24,0,10,3a10.18,10.18,0,0,1,3.78,8.46q0,.66-.12,2A17.25,17.25,0,0,1,354.08,419.25Zm-13.14,1a10.1,10.1,0,0,0,5.73-1.47,8.62,8.62,0,0,0,3.15-3.57,9.78,9.78,0,0,0,1-4.08,7.33,7.33,0,0,0-1.83-5q-1.83-2.1-5.61-2.1h-6.72l-2,16.2Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-16"
          d="M370.7,441a12.08,12.08,0,0,1-5-4.2,11.41,11.41,0,0,1-1.92-6.63,15.76,15.76,0,0,1,1.5-6.63,12.79,12.79,0,0,1,4.74-5.4A14.56,14.56,0,0,1,378.2,416a14.4,14.4,0,0,1,6.42,1.47,12,12,0,0,1,4.83,4.26,11.49,11.49,0,0,1,1.83,6.45,16.07,16.07,0,0,1-1.41,6.39,13.17,13.17,0,0,1-4.62,5.55,13.92,13.92,0,0,1-8.25,2.28A14.9,14.9,0,0,1,370.7,441Zm11.19-4.68a8.5,8.5,0,0,0,2.79-3.51,10.86,10.86,0,0,0,.9-4.32,8,8,0,0,0-1.14-4.41,7,7,0,0,0-2.91-2.61,8.26,8.26,0,0,0-3.57-.84,7.66,7.66,0,0,0-4.71,1.41,8.69,8.69,0,0,0-2.82,3.54,10.77,10.77,0,0,0-.93,4.35,7.88,7.88,0,0,0,1.11,4.32,7.08,7.08,0,0,0,2.88,2.61,8.27,8.27,0,0,0,3.69.87A7.66,7.66,0,0,0,381.89,436.32Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-17"
          d="M423.44,417l-3.06,24.84h-5.1l.24-4.62a12,12,0,0,1-4.41,4.05,12.34,12.34,0,0,1-6,1.53A7.34,7.34,0,0,1,400,441a6.05,6.05,0,0,1-1.95-4.71,5,5,0,0,1,.06-.9L400.28,417h5.64l-2,16.5a4.31,4.31,0,0,0,.81,3.24,4,4,0,0,0,3.15,1.26,7,7,0,0,0,3.9-1.23,9.76,9.76,0,0,0,3.09-3.39,12.42,12.42,0,0,0,1.53-4.8L417.8,417Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-18"
          d="M453.89,417.81a6.06,6.06,0,0,1,2,4.74,5,5,0,0,1-.06.9l-2.22,18.36H448l2-16.44a3.82,3.82,0,0,0-4.26-4.56,7.55,7.55,0,0,0-5.73,2.64,11.63,11.63,0,0,0-2.91,6.72l-1.44,11.64H430.1l3-24.84h5l-.06,4.14a12.57,12.57,0,0,1,4.44-3.72,13,13,0,0,1,6-1.38A7.75,7.75,0,0,1,453.89,417.81Z"
          transform="translate(-115.52 -367.37)"></path>
        <path class="cls-2 svg-elem-19"
          d="M495,395.31l-5.76,46.5h-5.58l.42-3.72q-4.26,4.26-9.18,4.26a12.26,12.26,0,0,1-6.27-1.62,11.8,11.8,0,0,1-4.38-4.38,11.93,11.93,0,0,1-1.59-6.06,14.52,14.52,0,0,1,1.77-7A13.95,13.95,0,0,1,477,416a12.41,12.41,0,0,1,5.4,1.17,10.81,10.81,0,0,1,4,3.09l3.06-24.9Zm-9.48,31.38a6.71,6.71,0,0,0-2.58-4.53,8.48,8.48,0,0,0-5.46-1.77,8.9,8.9,0,0,0-6.48,2.52,8.74,8.74,0,0,0-2.58,6.54,7.88,7.88,0,0,0,2.28,5.82,8,8,0,0,0,5.94,2.28,8.3,8.3,0,0,0,5-1.68,9.45,9.45,0,0,0,3.3-4.44Z"
          transform="translate(-115.52 -367.37)"></path>
        <style>
          .cls-1 {
            fill: #2d2943;
          }

          .cls-2 {
            fill: #232339;
          }
        </style>
      </svg>
    </div>
    <!-- /.svg-animation -->
    <header id="header" class="p-header">
      <div class="mobile-container">
        <div class="logo__img">
          <a class="animsition-link" href="/">
            <div class="logo"></div>
          </a>
        </div>
        <div class="mobile-button">
          <button class="mobile-menu__btn">
            <span></span>
            <span></span>
            <span></span>
          </button>
        </div>
      </div>
      <!-- /.mobile-container -->
      <div class="pc-container">
        <div class=" logo__img">
          <a class="animsition-link" href=" <?php echo esc_url( home_url('/')); ?>">
            <div class="logo-pc logo"></div>
          </a>
        </div>
        <nav class="pc-nav">
          <ul class="pc-nav-list">
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/promise')); ?>"><span>PROMISE</span></a>
            </li>
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/treatment')); ?>"><span>MENU</span></a>
            </li>
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/doctor')); ?>"><span>DOCTOR</span></a>
            </li>
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/news')); ?>"><span>NEWS</span></a>
            </li>
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/faq')); ?>"><span>FAQ</span></a>
            </li>
            <li class="pc-nav-item">
              <a class="pc-nav-link animsition-link" href=" <?php echo esc_url( home_url('/recruit')); ?>"><span>RECRUIT</span></a>
            </li>
          </ul>
        </nav>
      </div>
      <!-- /.pc-container -->
      <nav class="sp-navbtn">
        <ul class="sp-navbtn__container">
          <li><a class="tel" href="tel:092-686-7954">電話予約<i class="icon-tel"></i></a></li>
          <li><a class="net" target="_blank" href="https://beauty.hotpepper.jp/">オンライン予約<i class="icon-net"></i></a>
          </li>
        </ul>
      </nav>
      <!-- /.nav-btn -->
    </header>
    <!-- /.p-header -->
    <div id="main-contents">
      <section id="topview" class="p-topview child">
        <div class="hero">
          <div class="swiper-container">
            <div class="swiper-wrapper">
              <div class="swiper-slide">
                <img class="view-sp" src="<?php echo get_template_directory_uri(); ?>/images/home/clinic__1@sp.jpg" />
                <img class="view-pc" src="<?php echo get_template_directory_uri(); ?>/images/home/clinic__1@pc.jpg" />
              </div>
              <div class="swiper-slide">
                <img class="view-sp" src="<?php echo get_template_directory_uri(); ?>/images/home/medical__1@sp.jpg" />
                <img class="view-pc" src="<?php echo get_template_directory_uri(); ?>/images/home/medical__1@pc.jpg" />
              </div>
              <div class="swiper-slide">
                <img class="view-sp" src="<?php echo get_template_directory_uri(); ?>/images/home/reception@sp.jpg" />
                <img class="view-pc" src="<?php echo get_template_directory_uri(); ?>/images/home/reception@pc.jpg" />
              </div>
            </div>
          </div>
        </div>
      </section>
      <!-- /.top-view -->
      <section id="message" class="p-message">
        <div class="c-container">
          <h2 class="p-message__title clip-js left">
            <p class="c-head01">MESSAGE</p>
            <p class="c-sub-head01">メッセージ</p>
          </h2>
          <div class="p-message-wrap appear up">
            <div class="p-message__img01 cover-slide parallax">
              <img src="<?php echo get_template_directory_uri(); ?>/images/home/tool4@sp.jpg" alt="">
            </div>
            <div class="p-message__txtarea appear up">
              <h4 class="p-message__txt-tit c-head02 item">私は私が変えていく</h4>
              <p class="c-txt-sm item">大人の女性の、内 なる美しさ。<br class="view-pc">
                華美ではなく自然に醸し出る美しさ。<br class="view-pc">
                それは日々の健康と心の安寧から生まれる。<br class="view-pc">
                だれかに褒められたい、認められたい、ではなく、<br class="view-pc">
                あなたはあなた自身の人生を、自信をもって生きてゆこう。<br class="view-pc">
                この日を、この季節を、美しく、ポジティブに。<br class="view-pc">
                堂々と笑ってすごせるように。</p>
            </div>
          </div>
          <div class="p-message__sub-imgbox">
            <div class="p-message__sub-img01 parallax">
            </div>
            <div class="p-message__sub-img02 rellax">
            </div>
          </div>
        </div>
      </section>
      <!-- /#message -->
      <section id="promise" class="p-promise">
        <div class="c-container">
          <h2 class="p-promise__title clip-js left">
            <p class="c-head01">PROMISE</p>
            <p class="c-sub-head01">当医院の約束</p>
          </h2>
          <div class="p-promise__txtarea appear up">
            <h4 class="p-promise__txt-tit c-head02 item">しっかりと噛めること。<br class="view-pc">
              そして美しくあること。<br class="view-pc">
              「機能」「審美」のどちらも疎かにしない。</h4>
            <p class="c-txt-sm item">
              歯科治療は「医療」ですから、まずはしっかりと噛めること、発音できること、長持ちすることなど、機能面でベストな選択肢をご提案いたします。美しさのために「機能」を犠牲にしない。これが私たちの大前提です。その上で、みなさんがより良い人生を歩んでいけるよう、口もとが美しく、清潔に見える「審美」のお手伝いをいたします。下記3つのポイントを大切にしながら、「機能」と「審美」の両軸で患者さまのご要望に応えてまいります。
            </p>
          </div>
          <ul class="p-promise__list appear up">
            <li class="p-promise__item item">
              <span>01</span>
              <p>トータル治療と<br>ホームドクター</p>
            </li>
            <li class="p-promise__item item">
              <span>02</span>
              <p>最新技術による<br>高度医療を提供</p>
            </li>
            <li class="p-promise__item item">
              <span>03</span>
              <p>国内の有名な<br>歯科技工士との連携</p>
            </li>
          </ul>
          <div class="c-btn appear up">
            <a class="c-link animsition-link item" href=" <?php echo esc_url( home_url('/promise')); ?>">VIEW MORE</a>
          </div>
        </div>
      </section>
      <!-- /.p-promise -->
      <section id="treatments" class="p-treatments">
        <div class="p-treatments__img-wrap">
          <div class="p-treatments__img parallax"></div>
        </div>
        <div class="back-box parallax">
        </div>
        <div class="c-container">
          <h2 class="p-treatments__title clip-js left">
            <p class="c-head01">TREAT<br class="view-pc">MENTS</p>
            <p class="c-sub-head01">治療内容</p>
          </h2>
          <div class="p-treatments-wrap">
            <div class="p-treatments__txtarea appear up">
              <h4 class="p-treatments__txt-tit c-head02 item">一人ひとりに合った<br class="view-pc">治療をご提供します。</h4>
              <p class="c-txt-sm item">
                一般診療から最新の矯正装置を使ったスピード矯正、部分矯正まで。患者さまのご要望を伺いながら、口腔内の状態を診断し、一人ひとりに合った治療をご提示いたします。
              </p>
            </div>
            <div class="p-treatments__menu appear up">
              <ul class="p-treatments__list item">
                <li class="p-treatments__item"><span class="c-txt-sm">01</span>
                  <span class="c-txt-sm">虫歯・歯周病治療</span>
                </li>
                <li class="p-treatments__item"><span class="c-txt-sm">02</span>
                  <span class="c-txt-sm">ホワイトニング</span>
                </li>
                <li class="p-treatments__item"><span class="c-txt-sm">03</span>
                  <span class="c-txt-sm">インプラント</span>
                </li>
                <li class="p-treatments__item"><span class="c-txt-sm">04</span>
                  <span class="c-txt-sm">矯正</span>
                </li>
              </ul>
            </div>
          </div>
          <div class="c-btn appear up">
            <a class="c-link item animsition-link" href=" <?php echo esc_url( home_url('/treatment')); ?>">VIEW MORE</a>
          </div>
        </div>
      </section>
      <!-- /.p-treatments -->
      <section id="doctor" class="p-doctor">
        <div class="c-container">
          <h2 class="p-doctor__title clip-js left">
            <p class="c-head01">DOCTOR</p>
            <p class="c-sub-head01">医院長紹介</p>
          </h2>
          <div class="p-doctor__img-wrap parallax">
            <div class="p-doctor__img"></div>
          </div>
          <div class="p-doctor__contents appear up">
            <h4 class="p-doctor__name item">
              <p class="ja">利部　翔太</p>
              <p class="en">Shota Ribu</p>
              <p>医院長</p>
            </h4>
            <div class="p-doctor__wrap item">
              <h5>所属団体</h5>
              <ul class="p-doctor__list">
                <li class="p-doctor__item">噛み合わせ認定医</li>
                <li class="p-doctor__item">日本歯科医師会　会員</li>
                <li class="p-doctor__item">公益社団法人日本口腔インプラント学会　会員</li>
              </ul>
              <h5>経歴</h5>
              <ul class="p-doctor__list">
                <li class="p-doctor__item">2005年&emsp;福岡歯科大学歯学部卒業</li>
                <li class="p-doctor__item">2008年&emsp;医療法人Roundデンタルクリニック勤務</li>
                <li class="p-doctor__item">2009年&emsp;医療法人Round RivRoundデンタルクリニック開設</li>
              </ul>
            </div>
          </div>
          <div class="c-btn appear up">
            <a class="c-link animsition-link item" href="#">VIEW MORE</a>
          </div>
        </div>
      </section>
      <!-- /.p-doctor -->
      <section id="news" class="p-news">
        <div class="c-container">
          <h2 class="p-news__title clip-js left">
            <p class="c-head01">NEWS</p>
            <p class="c-sub-head01">お知らせ</p>
          </h2>
          <div class="p-news__wrap appear up">
            <ul class="p-news__list item">
              <?php query_posts('posts_per_page=3'); ?>
              <?php if(have_posts()) : while(have_posts()) : the_post(); ?>
                <li class="p-news__item">
                  <a href="<?php the_permalink() ;?>">
                    <span class="date"><?php echo get_the_date(); ?></span>
                    <p><?php the_title(); ?></p>
                  </a>
                </li>
              <?php endwhile; ?>
              <?php endif; ?>
            </ul>
          </div>
          <div class="c-btn appear up">
            <a class="c-link animsition-link item" href=" <?php echo esc_url( home_url('/news')); ?>">VIEW MORE</a>
          </div>
        </div>
      </section>
      <!-- /.p-news -->
      <section id="access" class="p-access">
        <div class="c-container">
          <h2 class="p-access__title clip-js left">
            <p class="c-head01">ACCESS</p>
            <p class="c-sub-head01">医院案内</p>
          </h2>
          <div class="p-access-container appear up">
            <div class="p-access-wrap item">
              <h3 class="p-access__name c-head02">
                RivRoundデンタルクリニック
              </h3>
              <div class="p-access__info">
                <ul class="p-access__list">
                  <li class="p-access__item">
                    <p>住所</p>
                    <p>〒812-0011</p>
                    <p>福岡市博多区博多駅前 3-37-25第二岡部ビル９階</p>
                  </li>
                  <li class="p-access__item">
                    <p>アクセス</p>
                    <p>博多駅博多口徒歩４分</p>
                  </li>
                  <li class="p-access__item">
                    <a class="p-access__tel" href="tel:092-686-8008">
                      <p>電話番号</p>
                      <p>092-686-8008</p>
                      </p>
                    </a>
                  </li>
                  <li class="p-access__item">
                    <p>診療科目</p>
                    <p>歯科 / 矯正歯科</p>
                  </li>
                  <li class="p-access__item">
                    <a class="p-access__g-map" href="https://goo.gl/maps/Ba1o85CQRCXAnAMb9" target="_blank">Google
                      Map</a>
                  </li>
                </ul>
              </div>
            </div>
            <div class="p-access__time item">
              <table>
                <tr>
                  <th>診療時間</th>
                  <th>月</th>
                  <th>火</th>
                  <th>水</th>
                  <th>木</th>
                  <th>金</th>
                  <th>土</th>
                  <th>日・<br>祝日</th>
                </tr>
                <tr>
                  <td>9:00〜13:00</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td class="close">／</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td class="close">／</td>
                </tr>
                <tr>
                  <td>14:00〜18:00</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td class="close">／</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td>◯</td>
                  <td class="close">／</td>
                </tr>
              </table>
            </div>
          </div>
        </div>
      </section>
      <!-- /.p-access -->
    </div>
    <!-------- /#main-contents -------->
    <footer id="footer" class="p-footer">
      <div class="p-footer__inner clip-text skew">
        <div class="p-footer__logo">
        </div>
        <div class="p-footer__menu">
          <ul class="p-footer__list">
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/promise')); ?>">PROMISE</a>
            </li>
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/treatment')); ?>">MENU</a>
            </li>
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/doctor')); ?>">DOCTOR</a>
            </li>
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/news')); ?>">NEWS</a>
            </li>
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/faq')); ?>">FAQ</a>
            </li>
            <li class="p-footer__item">
              <a class="animsition-link" href=" <?php echo esc_url( home_url('/recruit')); ?>">RECRUIT</a>
            </li>
          </ul>
        </div>
        <div class="p-footer__copy">
          &copy;2021 RivRound.inc
        </div>
      </div>
    </footer>
    <!-------- /.p-footer -------->
    <div class="mobile-menu">
      <nav class="mobile-menu__nav">
        <ul class="mobile-menu__list">
          <li class="mobile-menu__item">
            <a class="animsition-link" href=" <?php echo esc_url( home_url('/promise')); ?>">PROMISE</a>
          </li>
          <li class="mobile-menu__item">
            <a class="animsition-link" href="<?php echo esc_url( home_url('/treatment')); ?>">MENU</a>
          </li>
          <li class="mobile-menu__item">
            <a class="animsition-link" href="<?php echo esc_url( home_url('/doctor')); ?>">DOCTOR</a>
          </li>
          <li class="mobile-menu__item">
            <a class="animsition-link" href="<?php echo esc_url( home_url('/news')); ?>">NEWS</a>
          </li>
          <li class="mobile-menu__item">
            <a class="animsition-link" href="<?php echo esc_url( home_url('/faq')); ?>">FAQ</a>
          </li>
          <li class="mobile-menu__item">
            <a class="animsition-link" href="<?php echo esc_url( home_url('/recruit')); ?>">RECRUIT</a>
          </li>
        </ul>
        <div class="reserve-btn-wrap">
          <div class="reserve-btn-tel">
            <h4>電話予約はこちら</h4>
            <div class="reserve-btn">
              <a href="tel:092-686-7954"><span>092-686-7954</span></a>
            </div>
          </div>
          <div class="reserve-btn-net">
            <h4>ネット予約はこちら</h4>
            <div class="reserve-btn">
              <a href="https://beauty.hotpepper.jp/" target="_blank"><span>ONLINE RESERVE</span></a>
            </div>
          </div>
        </div>
      </nav>
    </div>
    <!------ /.mobile-menu ----->
  </div>
  <!-- /.superwrapper -->
  <!-- <script src="/scripts/vendor/pace.js"></script> -->
  <?php get_footer(); ?>
</body>

</html>